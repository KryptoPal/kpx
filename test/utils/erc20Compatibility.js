...
exports.test = function(web3, accounts, token) {
  describe('ERC20 Compatibility', function() {
    ...
    it('should register the "ERC20Token" interface with ERC820', async function() {
      let erc820RegistryAddr = await token.contract.methods
        .getIntrospectionRegistry()
        .call();
      let erc820Registry = utils.getERC820Registry(web3, erc820RegistryAddr);
    ...
    it(
      `should let ${utils.formatAccount(accounts[2])} ` +
        `transfer 1.5 ${token.symbol} ` +
        `to ${utils.formatAccount(accounts[1])}`,
      async function() {
    ...
    it(
      `should approve ${utils.formatAccount(accounts[3])} ` +
        `to send 3.5 ${token.symbol}` +
        ` from ${utils.formatAccount(accounts[1])}`,
      async function() {
        await utils.assertTotalSupply(web3, token, 10 * accounts.length);
        await utils.assertBalance(web3, token, accounts[1], 10);
        await utils.assertBalance(web3, token, accounts[2], 10);
    ...
    it(
      `should not let ${utils.formatAccount(accounts[3])} transfer from ` +
        `${utils.formatAccount(accounts[1])} (not approved)`,
      async function() {
        await utils.assertTotalSupply(web3, token, 10 * accounts.length);
        await utils.assertBalance(web3, token, accounts[1], 10);
        await utils.assertBalance(web3, token, accounts[2], 10);
