...
exports.test = function(web3, accounts, token) {
  describe('operator', function() {
    ...
    it(
      `should detect ${utils.formatAccount(accounts[3])} is not an operator ` +
        `for ${utils.formatAccount(accounts[1])}`,
      async function() {
        assert.isFalse(
    ...
    it(
      `should authorize ${utils.formatAccount(accounts[3])} as an operator ` +
        `for ${utils.formatAccount(accounts[1])}`,
      async function() {
        await token.contract.methods
    ...
    it(
      `should revoke ${utils.formatAccount(accounts[3])} as an operator for ` +
    ...
    it(
      `should not let ${utils.formatAccount(accounts[3])} authorize himself ` +
        'as one of his own operators',
      async function() {
        await utils.assertTotalSupply(web3, token, 10 * accounts.length);
    ...
    it(
      `should make ${utils.formatAccount(accounts[3])} ` +
        'an operator for himself by default',
      async function() {
        assert.isTrue(
          await token.contract.methods
    ...
    it(
      `should not let ${utils.formatAccount(accounts[3])} revoke himself ` +
        'as one of his own operators',
      async function() {
        await utils.assertTotalSupply(web3, token, 10 * accounts.length);
        await utils.assertBalance(web3, token, accounts[3], 10);
