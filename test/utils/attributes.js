...
exports.test = function(web3, accounts, token) {
  describe('attributes', function() {
    ...
    it(`should have a granularity of ${token.granularity}`, async function() {
      const granularity = (await token.contract.methods
        .granularity()
        .call()).toString();
      assert.strictEqual(web3.utils.fromWei(granularity), token.granularity);
    });

    it(`should have a total supply of ${token.totalSupply}`, async function() {
      await utils.assertTotalSupply(web3, token, token.totalSupply);
    });

    it(`should have a balance of ${
      token.defaultBalance
    } for all accounts`, async function() {
